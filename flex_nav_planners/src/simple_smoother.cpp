...
//
// The code is modified to store the max_timeout as internal parameter, and take the costmap
// pointer as argument instead of using a costmap subscriber.
// This version also removes the latest smoother_exceptions as they are not part of current
...
{
  double smoother_timeout_s = 1.0;
  auto node = parent.lock();
  std::string name = "simple_smoother";
  ...
  declare_parameter_if_not_declared(
    node, name + ".smoother_timeout", rclcpp::ParameterValue(smoother_timeout_s));
  ...
  node->get_parameter(name + ".smoother_timeout", smoother_timeout_s);

  // Convert timeout in seconds to duration
  int32_t seconds = static_cast<int>(smoother_timeout_s);
  int32_t nanoseconds = static_cast<int>((smoother_timeout_s - seconds)*1e9);
  max_time_ = rclcpp::Duration(seconds, nanoseconds);
  RCLCPP_INFO(logger_, "Created the simple smoother '%s' with timeout = %.6f seconds",
                       name.c_str(), max_time_.seconds());
  RCLCPP_INFO(logger_, "    parameters=(tol=%g, its=%d, data=%.3f, smooth=%.3f, do=%d, num=%d)",
                        tolerance_, max_its_, data_w_, smooth_w_, do_refinement_, refinement_num_);
}
  ...
  const nav2_costmap_2d::Costmap2D * costmap)
{
  ...
  double time_remaining = max_time_.seconds();
  ...
  for (unsigned int i = 0; i != path_segments.size(); i++) {
    if (path_segments[i].end - path_segments[i].start > 9) {
      ...
      time_remaining = max_time_.seconds() - duration_cast<duration<double>>(now - start).count();
      refinement_ctr_ = 0;
  ...
  const double & max_time)
{
  ...
  while (change >= tolerance_) {
    ...
    if (timespan > max_dur) {
      ...
      path = last_path;
      updateApproximatePathOrientations(path, reversing_segment);
      throw std::runtime_error("Smoothing time exceed allowed duration");
    }
